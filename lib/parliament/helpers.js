...
module.exports = {
    ...
    generateEventMessage: function (events, handlers, callback, done) {
        ...
        if (has_events) {
            ...
            handlers.handler.response.sessionAttributes['STATE'] = "";
            handlers.emit(":saveState", true);
    ...
    clearState: function (handlers) {
        handlers.handler.state = null;
        handlers.handler.response.sessionAttributes['STATE'] = "";
        handlers.emit(":saveState", true);
    },

    getState: function (handlers) {
        if(handlers.hander.state === null) {
            handlers.handler.state = "";
        }

        return handlers.handler.state;
    }
};