...
module.exports = {
                    ...
                    rp(uri_options.future)
                        .then(function(body){
                            ...
                            if(future_data.length > 0) {
                                let first_event = future_data[future_data.length-1];

                                // Does the following:
                                // 2017-06-13T00:00:00 => ['2017-06-13', '00:00:00'] => '2017-06-13'
                                events.future = first_event['StartDate'].split("T")[0];
                            }
    ...
    getHouseIntentData: function (intent) {
        ...
        return houseDataFilled ? intent.slots.house.value.toLowerCase() : null;
    },
    ...
    generateEventMessage: function (events, handlers, callback, done) {
        ...
        if (has_events) {
            ...
            if(events.future) {
                ...
                spoken_message.push('<say-as interpret-as="date" format="ymd">'+events.future+'</say-as>.');
                message.push(events.future + ".");
            }
