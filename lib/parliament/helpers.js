...
module.exports = {
     ...
     * let callback = function(events) { winston.log("Got events: " + events); };
    ...
    getEvents: function (uri_options, callback) {
        rp(uri_options.main)
            .then(function (body) {
                ...
                if( events.count <= 0 ) {
                    rp(uri_options.future)
                        .then(function(body){
                            ...
                            winston.log("E: WhatsOnIntent FUTURE_DATA Error: " + err.name ? err.name : "");
                            winston.log(err);
                ...
                winston.log("E: WhatsOnIntent Error: " + err.name ? err.name : "");
                winston.log(err);
    ...
    generateVenueEventList: function(venue_order, venues, future=false) {
        winston.log('Generate event string');

        let messageArray = [];

        venue_order.forEach((targetVenue) => {
            let venue;

            // There is/are
            if(venue = venues[targetVenue]) {
                // Are we talking about future events?
                if(future) {
                    messageArray.push('there will be');
                } else {
                    messageArray.push('There');
                    messageArray.push((venue.count === 1) ? 'is' : 'are');
                }

                messageArray.push(venue.count);
                messageArray.push(targetVenue);
                messageArray.push((venue.count === 1) ? 'event.' : 'events.');

                if(targetVenue === 'Select & Joint Committees') {
                    let eventStruct = {};

                    // Count the types of event
                    venue.events.forEach((event) => {
                        if(!eventStruct[event.title]) {
                            eventStruct[event.title] = 1
                        } else {
                            eventStruct[event.title] = eventStruct[event.title] + 1
                        }
                    });

                    let eventTypes = Object.keys(eventStruct);
                    messageArray.push(`Including${(eventTypes.length > 1) ? ',' : ''}`)

                    eventTypes.forEach((eventType, index) => {
                        let endEventWith = (eventTypes.length > 1) ? ',' : '.'; // We should put a comma at the end of each message in a list

                        // If it's the last in a chain, add an 'and' i.e. 13 Oral Evidence sessions and 3 Private meetings
                        if(eventTypes.length > 1 && index === (eventTypes.length - 1)) {
                            endEventWith = '.';
                            messageArray.push('and');
                        }

                        let eventString = (eventStruct[eventType] === 1) ? 'event' : 'events';
                        messageArray.push(`${eventStruct[eventType]} ${eventType} ${eventString}${endEventWith}`);
                    })
                } else {
                    venue.events.forEach((event) => {
                        let timeMessage = '';
                        if(event.startTime && event.endTime) {
                            timeMessage = `From ${event.startTime}, until ${event.endTime},  `;
                        } else if(event.startTime) {
                            timeMessage = `Starting at ${event.startTime}, `;
                        }

                        let descriptionMessage = '';
                        if(event.description && event.description.length > 0 && event.description.trim().length > 0) {
                            descriptionMessage = `, <s>${event.description}.</s>`;
                            descriptionMessage = descriptionMessage.replace(/(\r\n\r\n|\r\n)/g, '<break time="0.25s" />')
                        }


                        messageArray.push(`<p><s><prosody pitch="low">${timeMessage}${event.title}</prosody></s>${descriptionMessage}</p>`);
                    });
                }
            }
        });
