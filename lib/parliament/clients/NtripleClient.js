...
class NtripleClient {
    ...
    __handleNtripleAPIRequest(requestOptions, fulfill, reject) {
        ...
        Https.get(requestOptions, (response) => {
            ...
            winston.info(`NTriple API responded with a status code of : ${response.statusCode}`);
            ...
            response.on('data', (chunk) => {
                winston.info(`Received data from: ${response.responseUrl}`);

                data_string += chunk;
            });

            response.on('end', () => {
                winston.info(`Finished receiving data from: ${response.responseUrl}`);
                ...
                parser.parse(data_string, function(error, triple, prefixes){
                    if(triple) {
                        ...
                        winston.info('Parsing completed; Triple store contains %d triples.', store.size);
