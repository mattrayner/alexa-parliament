...
const AlexaNtripleClient = require("../AlexaNtripleClient");
        ...
        deviceAddressRequest.then((addressResponse) => {
            switch(addressResponse.statusCode) {
                case 200:
                    ...
                    const alexaNtripleClient = new AlexaNtripleClient(NTRIPLE_ENDPOINT);

                    let uri_postcode = postcode.replace(' ', '%20');
                    // let ssml_postcode = postcode.replace(' ', '').toLowerCase().split('').join('. ');
                    let ssml_postcode = postcode;
                    let ntripleRequest = alexaNtripleClient.getTripleStore(`/postcodes/${uri_postcode}`);
