...
const AlexaJSONClient = require("../AlexaJSONClient");
...
module.exports = {
    ...
    "WhatsOnIntent": function () {
        ...
        const alexaJSONClient = new AlexaJSONClient(CALENDAR_ENDPOONT, false);
        let calendarDataPath = '/calendar/events/list.json?date=30days';
        if(houseOfInterest) { calendarDataPath += `&house=${houseOfInterest}`; }
        let jsonRequest = alexaJSONClient.getJSON(calendarDataPath);
        ...
        jsonRequest.then((jsonResponse) => {
            switch(jsonResponse.statusCode) {
                case 200:
                    ...
                    let nonsittingJsonRequest = alexaJSONClient.getJSON('/calendar/events/nonsitting.json?date=today');
