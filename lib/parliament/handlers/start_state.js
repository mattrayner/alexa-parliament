...
module.exports = {
    ...
    "MyMPIntent": function() {
        ...
        if(!consentToken) {
            ...
            winston.info("User did not give us permissions to access their address.");
        ...
        deviceAddressRequest.then((addressResponse) => {
            switch(addressResponse.statusCode) {
                        ...
                        switch(ntripleResponse.statusCode) {
                            case 200:
                                ...
                                if(ntripleResponse.store.size === 0) {
                                    winston.info('No triples in store');
                                    return this.emit(':tell', `I can't find an MP for ${ssml_postcode}.`);
                                }

                                let store = ntripleResponse.store;

                                let memberTriple = store.getTriples(null, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'https://id.parliament.uk/schema/Person')[0];
                                if(memberTriple) {
                                ...
                                let partyTriple = store.getTriples(null, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'https://id.parliament.uk/schema/Party')[0];
                                if(partyTriple) {
                                    let partyNameTriple = store.getTriples(partyTriple.subject, "https://id.parliament.uk/schema/partyName", null)[0];
                                    if(partyNameTriple) { object.party = N3Util.getLiteralValue(partyNameTriple.object); }
                                }

                                let incumbencyTriple = store.getTriples(null, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'https://id.parliament.uk/schema/SeatIncumbency')[0];
                                if(incumbencyTriple) {
                                    let incumbencyStartDateTriple = store.getTriples(incumbencyTriple.subject, "https://id.parliament.uk/schema/incumbencyStartDate", null)[0];
                                    if(incumbencyStartDateTriple) { object.incumbency = moment(N3Util.getLiteralValue(incumbencyStartDateTriple.object), "YYYY-MM-DDZ").format("DD-MM-YYYY"); }
                                }

                                let constituencyTriple = store.getTriples(null, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'https://id.parliament.uk/schema/ConstituencyGroup');
                                if(constituencyTriple) {
                                    let constituencyNameTriple = store.getTriples(constituencyTriple.subject, "https://id.parliament.uk/schema/constituencyGroupName", null)[0];
                                    if(constituencyNameTriple) { object.constituency = N3Util.getLiteralValue(constituencyNameTriple.object); }
                                }

                                winston.info('Triples processed into data');
                ...
                case 204:
                    ...
                    winston.info("Successfully requested from the device address API, but no address was returned.");
                    this.emit(":tell", this.t('NO_ADDRESS'));
                    break;
                case 403:
                    winston.info("The consent token we had wasn't authorized to access the user's address.");
                    this.emit(":tellWithPermissionCard", this.t('NOTIFY_MISSING_PERMISSIONS'), PERMISSIONS);
                    break;
                default:
