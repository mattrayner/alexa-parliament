/**
 * ADDITIONS IN LINE WITH LICENSE:
 * - Lines 72-75 added to allow 500 responses to be handled within the application
 */
'use strict';
...
class AlexaDeviceAddressClient {
    ...
    getCountryAndPostalCode() {
        console.log('getCountryAndPostalCode()');
    ...
    __handleDeviceAddressApiRequest(requestOptions, fulfill, reject) {
        ...
        Https.get(requestOptions, (response) => {
            ...
            if (('' + response.statusCode).match(/^5\d\d$/))
                fulfill({ statusCode: response.statusCode, address: null });
